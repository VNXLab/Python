        ...
        try {
            ...
            if (($message.result.Count -gt 0) -and ($message.result.Count -gt $offset)) {
                ...
                if ($user_id -notmatch $telegram_id) {
                    $unauth_user_found = ('Użytkownik [{0}] {1} nieupoważniona osoba wysłała do bota następujące polecenie: {2}' -f $user_id, $username, $text)
                    SendMessage $unauth_user_found
                }
 
                if ($text -match "/online")          { SendMessage "Sesja aktywna" $text }
